# File: T015_Residual_ASPP_CBAM_Skips.py
import torch
import torch.nn as nn
from .generator_blocks import ResidualBlock, UpBlockCBAMSkip, ASPP_Module
...
class Generator(nn.Module):
    def __init__(self, input_nc=3, output_nc=3, ngf=64):
        ...
        # ASPP no Bottleneck
        self.bottleneck = ASPP_Module(in_ch=ngf*8*2, out_ch=ngf*16)
        # CBAM nos Skips + Residual
        self.dec4 = UpBlockCBAMSkip(ngf*16, ngf*8*2, ngf*8, ResidualBlock)
        self.dec3 = UpBlockCBAMSkip(ngf*8,  ngf*4*2, ngf*4, ResidualBlock)
        self.dec2 = UpBlockCBAMSkip(ngf*4,  ngf*2*2, ngf*2, ResidualBlock)
        self.dec1 = UpBlockCBAMSkip(ngf*2,  ngf*1*2, ngf,   ResidualBlock)
