# File: T012_Residual_SelfAttn_Bottleneck.py
import torch
import torch.nn as nn
from .generator_blocks import ResidualBlock, UpBlockBaseline, NonLocalBlock
...
class Generator(nn.Module):
    def __init__(self, input_nc=3, output_nc=3, ngf=64):
        ...
        # Usa ResBlock + NonLocalBlock
        self.bottleneck_conv = ResidualBlock(ngf*8*2, ngf*16)
        self.bottleneck_attn = NonLocalBlock(ngf*16)
        self.dec4 = UpBlockBaseline(ngf*16, ngf*8*2, ngf*8, ResidualBlock)
        self.dec3 = UpBlockBaseline(ngf*8,  ngf*4*2, ngf*4, ResidualBlock)
        self.dec2 = UpBlockBaseline(ngf*4,  ngf*2*2, ngf*2, ResidualBlock)
        self.dec1 = UpBlockBaseline(ngf*2,  ngf*1*2, ngf,   ResidualBlock)
    ...
    def forward(self, part1, part2):
        ...
        bottleneck = self.bottleneck_attn(bottleneck) # Aplica atenção
